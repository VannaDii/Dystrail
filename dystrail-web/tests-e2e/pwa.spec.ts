...
test('all assets, saved play and town scenes work after an offline relaunch',async({baseURL})=>{
 ...
 atTown(gs,'Madison');await importState(offline,gs);await offline.getByRole('button',{name:/^Talk to locals/}).click();await expect(offline.locator('.local-fact')).toContainText('Mendota');await snap(offline,'offline-town');
