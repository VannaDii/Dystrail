...
export async function baseline(page:Page,deep=false) {await setup(page,deep);await depart(page);await page.getByRole('button',{name:'Step mode',exact:true}).click();return savedState(page);}
...
export async function settle(page:Page) {
  ...
  if(await page.locator('.map-scene').count())await page.getByRole('button',{name:'Continue the journey',exact:true}).click();
}
