...
test('menu controls, pointer cursors and help stay inside viewport at either edge',async({page})=>{
 await baseline(page);await page.getByRole('tab',{name:'Conditions',exact:true}).click();await openMenu(page);
 await expect(page.getByRole('switch',{name:'High contrast',exact:true})).toBeVisible();await expect(page.getByRole('button',{name:'Save',exact:true})).toBeVisible();
 ...
 for(const label of ['How this works: Supplies','How this works: Allies','How this works: Weather effects']){
  const area=page.locator(label==='How this works: Weather effects'?'.conditions-hud':'.resource-hud');const trigger=area.getByRole('button',{name:label,exact:true});await trigger.click();
