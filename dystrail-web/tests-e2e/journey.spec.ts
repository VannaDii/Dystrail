...
test('outfitting recovery, direct controls, timed travel, manual save and camp',async({page})=>{
 ...
 await page.reload();await waitForLaunch(page);await expect(page.locator('.store-cart-summary .cart-total')).toHaveText('$88');
 expect(await page.evaluate(()=>document.activeElement?.tagName)).toBe('BODY');
 await snap(page,'outfitting');
 await page.getByRole('button',{name:'Review & depart',exact:true}).filter({visible:true}).first().click();
 await page.reload();await waitForLaunch(page);await expect(page.getByRole('button',{name:'Start the journey',exact:true})).toBeVisible();
 ...
 await page.reload();await waitForLaunch(page);await expect(page.locator('#main')).not.toHaveAttribute('data-screen','traveling');
 ...
 if(await page.locator('.encounter-choice button').count()) {
  ...
  await page.reload();await waitForLaunch(page);await expect(page.locator('.aftermath-panel')).toBeVisible();
