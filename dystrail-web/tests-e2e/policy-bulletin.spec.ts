...
const policyCopy=JSON.parse(readFileSync('i18n/en.json','utf8')).visual_copy;
...
function bulletin(base:any,index:number){
 ...
 gs.visual_content ??= {edition:1};gs.visual_content.policy_bulletins=[{id:`policy/${gs.logs.length-1}`,order:orders[index],unit:`${families[index]}-A`,received_day:gs.day,received_minute:gs.clock_minutes,acknowledged:false}];
 return gs;
}
...
test('all eighteen variants use their source copy and retained art without changing state',async({page},info)=>{
 test.setTimeout(90000);
 await page.setViewportSize({width:info.project.name==='mobile'?320:1440,height:1000});
 const base=await baseline(page);
 for(let i=0;i<orders.length;i++)for(const variant of ['A','B','C']){
  const gs=bulletin(base,i);const unit=`${families[i]}-${variant}`;
  gs.visual_content.policy_bulletins[0].unit=unit;
  gs.crew_care.pending='staffer';gs.crew_care.reason=0;gs.crew_care.strain={staffer:1};
  await importState(page,gs);
  await expect(page.locator('.policy-bulletin')).toHaveAttribute('data-unit',unit);
  await expect(page.locator('#bulletin-title')).toHaveText(policyCopy[unit].title);
  await expect(page.locator('.bulletin-copy')).toHaveText(policyCopy[unit].activation);
  await expect(page.locator('#bulletin-title')).toBeFocused();
  await expect(page.locator('.bulletin-illustration svg')).toHaveAttribute('data-atlas',`policy-bulletins-${unit==='ORDER-MILITARIZE-B'?'a':variant.toLowerCase()}`);
  await expect(page.locator('.bulletin-body>.policy-details')).toHaveCount(0);
  await expect(page.locator('.policy-indicator')).toBeVisible();
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
  const before=await current(page);expect(before.stats).toEqual(gs.stats);
  if(unit==='ORDER-TARIFFS-B'||unit==='ORDER-SHUTDOWN-C'||unit==='ORDER-GAG-A')await snap(page,`policy-final-${unit}`);
  await page.locator('#bulletin-continue').click();
  await expect(page.locator('#main')).toHaveAttribute('data-screen','crew-care');
  const after=await current(page);expect(after.stats).toEqual(before.stats);expect(after.rng_bundle).toEqual(before.rng_bundle);
 }
