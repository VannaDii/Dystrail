...
test('player and crew names recover during onboarding and absent members leave their seats',async({page})=>{
 ...
 await page.getByRole('tab',{name:'The Van',exact:true}).click();await expect(page.locator('.van-crew')).toContainText('Died');await expect(page.locator('.van-crew')).toContainText('Left the crew');await page.reload();await expect(page.locator('.standing-member')).toHaveCount(4);await snap(page,'crew-absences');
