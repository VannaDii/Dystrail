...
test('focus stays still, keyboard selection, rich locale, contrast and outside dismissal',async({page})=>{
 ...
 await expect(page.locator('html')).toHaveAttribute('dir','rtl');await page.reload();await expect(page.locator('html')).toHaveAttribute('lang','ar');await snap(page,'rtl');
 ...
 await page.getByRole('switch').click();await expect(page.getByRole('switch')).toHaveAttribute('aria-checked','true');await expect(page.locator('html')).toHaveClass(/hc/);
 ...
 await page.reload();await expect(page.getByRole('radio',{name:'Organizer',exact:true})).toHaveAttribute('aria-checked','true');
