...
import {baseline,depart,importState,savedState,setup,snap,openMenu} from './helpers';
...
test('starter allocation is paid, editable, emptyable and persisted',async({page})=>{
 await setup(page);await expect(page.locator('.loadout-panel')).toContainText('$71.00');
 await page.getByRole('button',{name:'Empty the van',exact:true}).click();
 await expect(page.locator('.loadout-items li')).toHaveCount(0);await page.reload();await expect(page.locator('.loadout-items li')).toHaveCount(0);
...
test('cashless breakdown has an escape with explicit costs across midnight',async({page})=>{
 ...
 const after=await savedState(page);expect(after.breakdown).toBeNull();expect(after.budget_cents).toBe(0);expect(after.stats.sanity).toBe(4);expect(after.stats.morale).toBe(5);expect(after.day).toBe(gs.day+1);expect(after.clock_minutes).toBe(180);await snap(page,'radio-repair');
});
test('foraging and paid town work apply costs and persist their limits',async({page})=>{
 ...
 await page.getByRole('button',{name:'Forage with a local guide',exact:true}).click();const foraged=await savedState(page);expect(foraged.stats.supplies).toBe(10);expect(foraged.clock_minutes).toBe(gs.clock_minutes+120);
 await page.getByRole('button',{name:'Back to the road',exact:true}).click();await page.reload();await expect(page.getByRole('button',{name:'Forage with a local guide',exact:true})).toBeDisabled();await expect(page.locator('.action-availability')).toContainText('3 travel days');
 ...
 await page.getByRole('button',{name:'Continue',exact:true}).click();await page.reload();await expect(page.getByRole('button',{name:'Take a paid unloading shift',exact:true})).toBeDisabled();
});
test('town facts and player and NPC portraits survive reload',async({page})=>{
 ...
 await page.getByRole('button',{name:/^Talk to locals/}).click();await expect(page.locator('.local-fact')).toContainText('Mendota');
 ...
 const npc=await page.locator('.scene-npc').getAttribute('data-npc');await page.reload();await expect(page.locator('.scene-npc')).toHaveAttribute('data-npc',npc!);
 await expect(page.locator('.local-conversation blockquote')).toContainText('showerhead');await snap(page,'town-conversation');
});
test('a new political encounter pays cash and exposes its factual hook',async({page})=>{
 ...
 await expect(page.locator('.journey-scene')).toHaveAttribute('data-scene','enc-radio');await page.getByRole('button',{name:'How this works: Behind the joke',exact:true}).click();await expect(page.locator('.viewport-help')).toContainText('five weekly');await expect(page.locator('.viewport-help a')).toHaveAttribute('href',/^https:/);await page.keyboard.press('Escape');
...
test('road and progress elements survive consecutive automatic turns',async({page})=>{
 ...
 await importState(page,gs);await page.getByRole('button',{name:'Fast',exact:true}).click();
