...
pub fn journey_scene(p: &Props) -> Html {
    ...
    let authored = barter_ratio.as_deref().or_else(|| ally_art::context(&p.stage).map(|_| "1.5")).or_else(|| crossing.then_some("1.5")).or_else(|| care_art::context(&p.stage).map(|_| "1.5")).or_else(|| road_art::aspect(&p.stage));
    let authored_style =
