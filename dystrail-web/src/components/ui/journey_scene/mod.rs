...
pub enum SceneStage {
    ...
    Crossing { unit: String, passed: bool },
    Ending(bool),
    Breakdown,
    Encounter(String),
...
pub fn asset_name(deep: bool, stage: &SceneStage) -> Option<&'static str> {
    match stage {
        ...
        SceneStage::Crossing { .. } => Some("crossing"),
...
pub fn journey_scene(p: &Props) -> Html {
    ...
    let indoors = ally_art::context(&p.stage).map(|_| true).or_else(|| care_art::indoors(&p.stage)).unwrap_or_else(|| road_art::context(&p.stage).map_or_else(
        || name.is_some_and(composition::is_indoors),
        |(unit, _)| road_art::is_indoors(unit),
    ));
    let crossing = matches!(p.stage, SceneStage::Crossing { .. });
    let authored = ally_art::context(&p.stage).map(|_| "1.5").or_else(|| crossing.then_some("1.5")).or_else(|| care_art::context(&p.stage).map(|_| "1.5")).or_else(|| road_art::aspect(&p.stage));
    let authored_style =
    ...
    html! { <figure style={authored_style} data-weather={weather} data-time={light} data-hour={p.hour.to_string()} data-indoors={indoors.to_string()} data-scene={name.unwrap_or("unillustrated").to_owned()} class={classes!("journey-scene",authored.is_some().then_some("scene-authored"),road.then_some("scene-road"),p.moving.then_some("scene-moving"))}>
        <div class="scene-art" aria-hidden="true">
            ...
            if let Some(art)=ally_art::render(&p.stage) {{art}} else if let Some(art)=care_art::render(p) {{art}} else if let Some(art)=road_art::render(&p.stage) {{art}} else if let Some(art)=name.and_then(|name|composition::setting(p,name)) {{art}} else if road {<div class="road-pan-track">{for (0..2).map(|i|html!{<img class={classes!("scene-background",(i==1).then_some("road-mirrored"))} src={crate::paths::asset_path(&format!("static/img/journey/{}.png",name.unwrap_or("open-heartland-orchard")))} alt="" decoding="sync" width="1536" height="1024" />})}</div>} else if let Some(name)=name {<img class="scene-background" src={crate::paths::asset_path(&format!("static/img/journey/{name}.png"))} alt="" decoding="sync" width="1536" height="1024" />}
