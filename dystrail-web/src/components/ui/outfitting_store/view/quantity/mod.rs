...
pub fn render_quantity_screen(
    ...
    html! { <main class="outfitting-store"><section role="region" aria-labelledby="qty-title" onkeydown={on_keydown}><h1 id="qty-title">{ title }</h1>{ if current_qty > 0 { html! { <p class="current-qty">{ format!("Current in cart: {current_qty}") }</p> } } else { html! {} }}<div ref={list_ref}>{ render_quantity_options(&options, state.focus_idx, &on_select) }</div><div aria-live="polite" aria-atomic="true" class="sr-only" id="store-status"></div></section></main> }
}
