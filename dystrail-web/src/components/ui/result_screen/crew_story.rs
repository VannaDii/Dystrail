...
pub fn render(gs: &GameState) -> Html {
    html! {<section class="crew-epilogue" aria-labelledby="crew-epilogue-title">
        ...
        <ul class="ending-crew">{for gs.party.members.iter().map(|m|{
            ...
            html!{<li data-fate={key}><span class={classes!("portrait-art",format!("portrait-{}",m.persona))} aria-hidden="true"></span><div><strong>{&m.name}</strong><span>{i18n::t(key)}</span></div></li>}
        })}</ul>
        <h3>{i18n::t("journey.turning_points")}</h3><ol class="ending-moments">{for gs.continuity.journal.iter().rev().filter(|e|e.title!=i18n::t("play.last_turn")).take(4).map(|entry|html!{<li><strong>{format!("{} · {}",entry.day,entry.title)}</strong><p>{&entry.message}</p></li>})}</ol>
    </section>}
}
