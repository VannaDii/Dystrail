        ...
        let listener = if cfg!(target_arch = "wasm32") {
            ...
            let handler=Closure::wrap(Box::new(move |event:web_sys::KeyboardEvent| {
                ...
                if let Ok(nodes)=document.query_selector_all(&selector) {for i in 0..nodes.length(){
                    if let Some(el)=nodes.get(i).and_then(|n|n.dyn_into::<web_sys::HtmlElement>().ok()) && el.offset_width()>0 && !el.has_attribute("disabled") {event.prevent_default();if el.get_attribute("role").as_deref()==Some("radio"){let _=el.focus();}el.click();break;}
                }}
