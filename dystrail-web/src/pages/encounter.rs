...
pub fn encounter_page(props: &EncounterPageProps) -> Html {
    ...
    encounter.map_or_else(
        ...
        |enc| {
            html! {
                <>
                    <crate::components::ui::world_view::WorldView state={props.state.clone()} title={crate::i18n::encounter_text(&enc.id,"name",&enc.name)} stage={Some(SceneStage::Encounter(enc.id.clone()))} />
                    {props.receipt.clone()}
