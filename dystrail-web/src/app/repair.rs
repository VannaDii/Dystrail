...
pub fn render(app: &AppState) -> Html {
    ...
    html! {<section class="roadside-options" aria-labelledby="repair-title">
        ...
        <div class="action-grid">{for RepairChoice::ALL.into_iter().map(|choice|{
            ...
            html!{<div class="action-option"><button disabled={!gs.can_repair(choice)} onclick={choose(app,choice)}>{label}</button><span>{cost}{" · "}{i18n::t("play.vehicle")}{" +"}{i18n::fmt_number(f64::from(gain))}{"%"}</span></div>}
        })}</div>
