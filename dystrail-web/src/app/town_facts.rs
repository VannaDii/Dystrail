...
pub fn render(app: &super::state::AppState) -> Html {
    ...
    html! {<>
        ...
        <section class="local-conversation" aria-label={i18n::t("journey.local_word")}><p class="eyebrow">{fact.town.clone()}</p><p class="local-fact">{fact.message()}</p><blockquote>{fact.remark()} {crate::components::ui::satire_context::hook(&fact.satire_hook)}</blockquote><p class="fact-source">{i18n::tr("trail.verified",Some(&BTreeMap::from([("date",fact.checked.as_str())])))}{" · "}<a href={fact.source} target="_blank" rel="noopener noreferrer">{i18n::t("trail.source")}</a></p><p class="fact-note">{i18n::t("trail.fact_note")}</p><button class="retro-btn-primary" onclick={done}>{i18n::t("trail.listen")}</button></section>
    </>}
}
#[cfg(test)]
mod tests {
