...
use super::{Props, Region, road_asset};
use crate::game::state::CrossingOutcomeTelemetry as Outcome;
use yew::prelude::*;

pub fn render(p: &Props, unit: &str, outcome: Outcome, permit: bool) -> Option<Html> {
    let asset = match unit {
        "CROSS-01-A" => "crossing-laminate-v2".to_owned(),
        "CROSS-02D-A" => "crossing-privacy-v2".to_owned(),
        "CROSS-02C-A" => format!("crossing-workers-{}-v2", if outcome == Outcome::Passed { "raised" } else { "lowered" }),
        "CROSS-03-A" => format!("crossing-crown-{}-v2", match (outcome, permit) {
            (Outcome::Passed, true) => "permit", (Outcome::Passed, false) => "passed",
            (Outcome::Detoured, _) => "detoured", (Outcome::Failed, _) => "failed",
        }),
        _ => return None,
    };
    let region = road_asset(p.region.unwrap_or(Region::Heartland), p.day);
    ...
    Some(html! {
        <svg class="scene-background crossing-layered-art" viewBox="0 0 1536 1024" preserveAspectRatio="xMidYMid meet" data-route-art={region}>
            ...
            <image class="crossing-authored-outcome" data-permit={permit.to_string()} href={crate::paths::asset_path(&format!("static/img/scenes-v2/{asset}.png"))} width="1536" height="1024" clip-path={format!("url(#{props_id})")} />
        </svg>
    })
}
