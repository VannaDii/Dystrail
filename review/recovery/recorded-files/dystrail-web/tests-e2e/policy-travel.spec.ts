...
test('a queued automatic route preview remains available after a bulletin',async({page})=>{
 ...
 gs.visual_content.policy_bulletins=[{id:`policy/${gs.logs.length-1}`,order:'Shutdown',unit:'ORDER-SHUTDOWN-A',received_day:6,received_minute:gs.clock_minutes,acknowledged:false}];
 await importState(page,gs);
 await page.evaluate(()=>{
