...
async function load(page:any,stop:number,locale='en',capped=false){
 ...
 await page.reload();await waitForLaunch(page);await expect(page.locator('.town-arrival')).toBeVisible();return await current(page);
}
test('town forecasts are qualitative and replay retains one resident and one reward',async({page},info)=>{
 ...
 for(const stop of [280,478,1140]){
  ...
  for(const locale of ['en','es','it','ar']){
   ...
   await expect(button).toBeVisible();await expect(button.locator('.action-detail')).not.toContainText('+1');expect(await button.getAttribute('aria-description')).not.toMatch(/[+−]1/);
   await button.click();await expect(page.locator('.local-conversation')).toBeVisible();const after=await current(page);expect(after.clock_minutes).toBe(630);expect(after.rng_bundle).toEqual(before.rng_bundle);
   expect(after.stats.credibility-before.stats.credibility).toBe(stop===1140?1:0);expect(after.stats.allies-before.stats.allies).toBe(stop===478?1:0);expect(after.receipts.length-before.receipts.length).toBe(stop===280?1:0);
   if(expectedResident===undefined)expectedResident=after.activities.local_word;expect(after.activities.local_word).toBe(expectedResident);
   await page.reload();await waitForLaunch(page);expect(await current(page)).toEqual(after);
   await page.locator('.conversation-actions button').click();await expect(page.locator('.town-arrival')).toBeVisible();await page.locator('.route-stop .action-button').click();
 ...
 for(const stop of [478,1140]){
  const before=await load(page,stop,'en',true);await expect(page.locator('.route-stop .action-detail')).toContainText('Receipt');await page.locator('.route-stop .action-button').click();await expect(page.locator('.local-conversation')).toBeVisible();const after=await current(page);
...
test('mode selection explains the approved themes in translated and fallback copy',async({page},info)=>{
 ...
 for(const locale of Object.keys(expected)){
  await page.goto('./');await waitForLaunch(page);await page.evaluate(locale=>{localStorage.removeItem('dystrail.autosave.v1');localStorage.removeItem('dystrail.save.default');localStorage.setItem('dystrail.locale',locale);},locale);
  await page.reload();await waitForLaunch(page);const option=page.locator('.mode-option').nth(1);await expect(option).toContainText(expected[locale]);await option.locator('input').check();await expect(page.locator('#run-code')).toHaveValue(/^DP-/);
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);if(['en','ar'].includes(locale))await page.screenshot({path:info.outputPath(`mode-${locale}.png`),fullPage:true,animations:'disabled'});
 }
});
