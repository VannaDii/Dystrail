...
parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--inputs',type=Path,default=Path('docs/ux/map-sources'));root=parser.parse_args().inputs
def source(name):
    path=root/name
...
code_by_city={'Denver':'CO','North Platte':'NE','Omaha':'NE','Des Moines':'IA','Iowa City':'IA','Minneapolis':'MN','La Crosse':'WI','Madison':'WI','Kansas City':'MO','Columbia':'MO','St. Louis':'MO','Springfield, IL':'IL','Austin':'TX','Waco':'TX','Dallas':'TX','Oklahoma City':'OK','Tulsa':'OK','Joplin':'MO','Springfield, MO':'MO','Chicago':'IL','South Bend':'IN','Toledo':'OH','Cleveland':'OH','Pittsburgh':'PA','Cumberland':'MD','Hagerstown':'MD','Frederick':'MD','D.C.':'DC'}
routes=[]
for persona in ['journalist','organizer','whistleblower','lobbyist','staffer','satirist']:
    ...
    for i,name in enumerate(names):
        ...
        region='Beltway' if name in ['Frederick','D.C.'] else 'RustBelt' if i>=names.index('Chicago') else 'Heartland'
        scene='open-beltway-parkway' if name=='D.C.' else 'open-beltway-suburbs' if name=='Frederick' else 'open-appalachian-ridge' if name in ['Pittsburgh','Cumberland','Hagerstown'] else 'open-rustbelt-lakeside' if name in ['Toledo','Cleveland'] else 'open-rustbelt-foundry' if region=='RustBelt' else 'open-heartland-orchard' if name in ['Des Moines','Iowa City','La Crosse','Madison'] else 'open-heartland-prairie'
        stops.append({'name':name,'state':code_by_city[name],'mile':round(cumulative[cursor]),'x':x,'y':y,'lon':lon,'lat':lat,'region':region,'scene':scene})
