...
impl GameState {
    ...
    pub(crate) fn apply_travel_progress(&mut self, distance: f32, kind: TravelProgressKind) -> f32 {
        ...
        self.miles_traveled_actual = (before + distance.min(remaining)).min(self.trail_distance);
        // Credit the change the odometer can represent, not the unrounded request.
        let applied = self.miles_traveled_actual - before;
