...
const payload='/tmp/dystrail-hourly-balance-release/release-site';
...
assert.notEqual(expected,'23cf83a1a7d5f9d02de9','Do not verify an unstaged baseline as a new release');
assert.equal(manifest.revision,expected);
const before=JSON.parse(await readFile(out+'/old-production-autosave.json','utf8'));
// A fresh saved run gains only the two defaulted precise-mileage fields.
assert.equal(before.state.miles_traveled_actual,0);
for(const key of ['state','pending']) {
 for(const field of ['distance_remainder','day_start_remainder']) {
  assert.equal(Object.hasOwn(before[key],field),false);
  before[key][field]=0;
 }
}

// Refresh only these derived rules, each checked against the old and new frozen configs.
const ruleMigration=JSON.parse(await readFile(out+'/expected-config-migration.json','utf8'));
for(const change of ruleMigration){
 let parent=before.state;
 for(const key of change.path.slice(0,-1))parent=parent[key];
 const leaf=change.path.at(-1);
 assert.equal(parent[leaf],change.before);
 parent[leaf]=change.after;
}

const profile=out+'/recheck-old-production-profile';
...
try{
 ...
 // Preserve every existing hourly-save value plus the two explicit new zero-default fields.
 ...
 assert.deepEqual(ordered(saved),ordered(expectedSave),'Published game update changed the saved envelope beyond the explicit zero-default fields and frozen balance-rule changes');
 report.schemaChanges=['distance_remainder','day_start_remainder'].map(field=>({field,default:0}));report.ruleChanges=ruleMigration;report.addedStateKeys=['distance_remainder','day_start_remainder'];
 ...
 console.log('Updated production cache and prior save verified',JSON.stringify({revision:expected,assets:report.cache.assets,images:report.updated.images,ruleChanges:report.ruleChanges}));
 ...
 const vanTab=page.getByRole('tab',{name:'The Van',exact:true});
 if(await vanTab.getAttribute('aria-selected')!=='true')await vanTab.click();
