...
work = Path('/tmp/dystrail-hourly-balance-release-prep')
root = Path('/tmp/dystrail-hourly-balance-release')
payload = root/'release-site'
baseline = json.loads(Path('/tmp/dystrail-hourly-balance-release-prep/baseline-release-manifest.json').read_text())
...
protected = {name:sha for name,sha in before.items() if not name.startswith('play/') and name != '404.html'}
...
assert not removed
assert all(name.startswith('play/') or name == '404.html' for name in changed + added)
