...
const out='/tmp/dystrail-workshop-production-check';
const payload='/tmp/dystrail-workshop-release/release-site';
...
assert.notEqual(expected,'1c145d89fef4fbab75a9','Do not verify an unstaged baseline as a new release');
assert.equal(manifest.revision,expected);
const before=JSON.parse(await readFile(out+'/old-production-autosave.json','utf8'));
// The new fractional driving-fatigue field has an explicit zero default in a fresh saved run.
assert.equal(before.state.miles_traveled_actual,0);
for(const key of ['state','pending']) {
 assert.equal(Object.hasOwn(before[key],'pace_fatigue_remainder'),false);
 before[key].pace_fatigue_remainder=0;
}
...
try{
 ...
 // Preserve every existing hourly-save value plus the explicit new zero-default field.
 ...
 assert.deepEqual(ordered(saved),ordered(expectedSave),'Published game update changed the saved envelope beyond the explicit zero-default field');
 report.schemaChanges=[{field:'pace_fatigue_remainder',default:0}];report.ruleChanges=['pace fatigue follows actual driving time','gear illness protection','overnight action settlement','crossing and encounter arrival timing'];report.addedStateKeys=['pace_fatigue_remainder'];
