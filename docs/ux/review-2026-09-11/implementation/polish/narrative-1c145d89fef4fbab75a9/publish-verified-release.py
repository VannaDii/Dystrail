"""Publish the reviewed immutable narrative artifact; always remove its exact temporary policy."""
import datetime
import json
import subprocess
import time
...
out = Path('/tmp/dystrail-narrative-release-prep')
workspace = Path('/tmp/dystrail-narrative-release')
...
record = {'revision':'1c145d89fef4fbab75a9','branch':branch,'commit':commit,'started_at':datetime.datetime.now(datetime.timezone.utc).isoformat()}
...
assert latest == [{'conclusion':'success','headSha':'acaa88b1c1f104bf267048d393df2be403f056c9','status':'completed'}], 'Production baseline changed before dispatch'
temporary = None
try:
