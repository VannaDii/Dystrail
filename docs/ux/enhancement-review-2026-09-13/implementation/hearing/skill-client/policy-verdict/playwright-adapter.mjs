...
const checkpoint=JSON.parse(readFileSync(new URL('./checkpoint.json',import.meta.url),'utf8'));
export const chromium={launch:async options=>{
 ...
 browser.newPage=async options=>{
  ...
  await page.addInitScript(checkpoint=>{
   localStorage.setItem('dystrail.autosave.v1',JSON.stringify(checkpoint));
