...
try {
 for(const deep of [false]) for(const width of [1280]) {
  ...
  await page.locator('.conditions-hud').screenshot({path:'/tmp/dystrail-source-handoff-20260913/desktop-status-final.png',animations:'disabled'});
  const bounds=await page.locator('#main').boundingBox();writeFileSync('/tmp/dystrail-source-handoff-20260913/desktop-status-final.json',JSON.stringify({bounds,revision:await page.evaluate(()=>window.dystrailOffline.revision),errors},null,2));assert.deepEqual(errors,[]);await context.close();
 }
