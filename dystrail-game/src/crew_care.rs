...
mod tests {
    use super::*;
    #[test]
    fn losing_the_player_ends_the_journey_with_a_player_specific_message() {
        let mut gs = GameState {
            persona_id: Some("whistleblower".into()),
            ..GameState::default()
        };
        gs.party.initialize("whistleblower", 7);
        gs.continuity.crew_care.pending = Some("whistleblower".into());
        gs.continuity
            .crew_care
            .strain
            .insert("whistleblower".into(), 3);
        assert_eq!(gs.resolve_crew_care(2), Some("journey.player_lost"));
        assert_eq!(
            gs.ending,
            Some(crate::Ending::Collapse {
                cause: crate::CollapseCause::Disease
            })
        );
        assert!(
            !gs.party
                .occupants()
                .iter()
                .any(|(_, m)| m.persona == "whistleblower")
        );
    }
    #[test]
